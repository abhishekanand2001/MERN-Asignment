...
app.get('/initialize-database', async (req, res) => {
  try {
    ...
    transactions.forEach(transaction => {
      // Convert dateOfSale to a JavaScript Date object
...
app.get('/list-trasnsaction', (req, res) => {
  const { month, search = '', page = 1, per_page = 10 } = req.query;

  const filteredTransactions = database.filter(transaction =>
    DateTime.fromJSDate(transaction.dateOfSale).toFormat('MMMM').toLowerCase() === month.toLowerCase()
  );

  const searchResults = search
    ? filteredTransactions.filter(transaction =>
        transaction.product_title.toLowerCase().includes(search.toLowerCase()) ||
...
app.get('/statistics', (req, res) => {
  ...
  const selectedTransactions = database.filter(transaction =>
    DateTime.fromJSDate(transaction.dateOfSale).toFormat('MMMM').toLowerCase() === month.toLowerCase()
  );

  const totalSaleAmount = selectedTransactions.reduce((sum, transaction) => sum + transaction.price, 0);
  const totalSoldItems = selectedTransactions.length;
...
app.get('/combined-response', async (req, res) => {
  try {
    await axios.get('http://localhost:3000/initialize-database'); // Assuming the API is on the same server
    const listTransactionsResponse = await axios.get('http://localhost:3000/list-transactions', { params: req.query });
    const statisticsResponse = await axios.get('http://localhost:3000/statistics', { params: req.query });
    const barChartResponse = await axios.get('http://localhost:3000/bar-chart', { params: req.query });
    const pieChartResponse = await axios.get('http://localhost:3000/pie-chart', { params: req.query });

    const combinedResponse = {
      initialize_database: { message: 'Database initialized successfully' },
